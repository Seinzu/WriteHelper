 ...
 * @since 1.1
 */
class CustomJuiTabs extends CJuiWidget 
{
	/**
	 ...
	 */
	public $headerTemplate='<li id="{id}"><a href="{url}">{title}</a></li>';
	/**
	...
	public function run()
	{
		...
		foreach($this->tabs as $title=>$content)
		{
			...
			{
				$tabsOut .= strtr($this->headerTemplate,array('{title}'=>$title, '{id}'=>$tabId, '{url}'=>CHtml::normalizeUrl($content['ajax'])))."\n";
			}
		}
		echo "<ul>\n" . $tabsOut . "</ul>\n";
		echo $contentOut;
